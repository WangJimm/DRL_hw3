...
class Agent(object):
    ...
    def load_model(self, actor_path = "pol_model_v1.pth", critic_path = "cri_model_v1.pth"):
        #load model
...
###start training
agent = Agent(env.action_space) # assign agent
memory = ReplayMemory(1000000) #assign memo
load_models = True
save_models = True
save_memory = False
...
for epi in itertools.count(1):
    ...
    obs = env.reset()
    #print(f"obs_size:{obs.shape}")
    obs = obs[0]
    #print(f"obs_size after obs[0]:{obs.shape}")
    # gray scale
    obs = cv2.cvtColor(obs, cv2.COLOR_BGR2GRAY)
    obs = obs[40:80, 30:70]
    #print(f"obs_size after gray scale:{obs.shape}")
    ...
    while not done:
        ...
        if len(memory) > batch_size:
            for i in range(updates_per_step):
                critic_1_loss, critic_2_loss, policy_loss, ent_loss = agent.update_parameters(memory, batch_size, updates)
                updates += 1

        
        n_obs, reward, done, _ = env.step(action)
        #print(f"size of n_obs{n_obs.shape}")
        n_obs = n_obs[0]
        n_obs = cv2.cvtColor(n_obs, cv2.COLOR_BGR2GRAY)
        n_obs = n_obs[40:80, 30:70]
        
        #cv2.destroyAllWindows()
        ## some change on n_obs
        ##############
        #n_obs = process_observation(n_obs)
        #n_obs = agent.policy.sample(n_obs)
        ##############
        ep_s += 1
        ep_r += reward
        total_numsteps += 1
        if total_numsteps % 50 == 0:
            now = time.time()
            print(f"total_numstep:{total_numsteps}, spend {round(now-prev_time, 2)} sec")
            prev_time = time.time()
        
        mask = 1 if ep_s == env._max_episode_steps else float(not done)
        
        memory.push(obs, action, reward, n_obs, mask)

        obs = n_obs
    if total_numsteps > num_steps:
        break
    print(f"Episode: {epi}, total numsteps: {total_numsteps}, reward: {round(ep_r[0], 2)}")
    ...
    if epi % eval_interval == 0:
        ...
        for i in range(episodes_2):
            print(f"test, {i}/{episodes_2}")
            obs = env.reset()
            ### do something to obs
            obs = obs[0]
            obs = cv2.cvtColor(obs, cv2.COLOR_BGR2GRAY)
        ...
        #        agent.save_model()
                ...
                #memory.save()
        print("-"*40)
        print(f"Test Episodes: {episodes_2}, Avg. Reward: {round(avg_r, 2)}")
        print("-"*40)
        reward_plot.append(avg_r)
    
            
env.close()

#plot reward
plt.plot(reward_plot,label = "reward")
plt.legend()
plt.show()
